...
def evaluate(model, data_loader):
    ...
    for data in data_loader:
        with torch.no_grad():
            ...
            # criterion = torch.nn.BCELoss(weight = (y_true*2).add(1))
...
def cross_validation(all_dataframe, fold_number=5):
    ...
    for train_index, valid_index in kfold.split(sequence_names, sequence_labels):
        ...
        model = DHEGPPIS(LAYER, INPUT_DIM, HIDDEN_DIM, NUM_CLASSES, DROPOUT, LAMBDA, ALPHA)
        if torch.cuda.is_available():
            model.cuda()
...
def train_full_model(all_dataframe, aver_epoch):
    ...
    for epoch in range(NUMBER_EPOCHS):
        ...
        if epoch % 10 == 0 and hasattr(model.subegnn, 'get_feature_importance'):
            feature_importance = model.subegnn.get_feature_importance()
            if feature_importance is not None:
                feature_names = ['PSSM', 'HMM', 'DSSP', 'Atom', 'Dist']
                print(f"\nEpoch {epoch} - Feature Importance:")
                for name, importance in zip(feature_names, feature_importance):
                    print(f"  {name}: {importance:.4f}")
        aver_epoch = 1
        if epoch + 1 >= aver_epoch :
