...
# model parameters
# ADD_NODEFEATS = 'all'  # all/atom_feats/psepose_embedding/no/newall
ADD_NODEFEATS = 'all'    
...
class GNet(nn.Module):
    def __init__(self, in_dim, n_classes, args = {'drop_n':0.0 ,'ks': [1,75], 'l_dim':62,'h_dim':62}):
        ...
        self.out_l_1 = nn.Linear(args['l_dim'], 32)
        self.out_l_2 = nn.Linear(32, 16)
        # self.out_l_3 = nn.Linear(10, n_classes)
        # self.out_drop = nn.Dropout(p=0.0)
        self.conv_classifier = nn.Sequential(
            nn.Conv1d(16, 16, kernel_size=3, padding=1, groups=8),  # 分组卷积
            nn.ReLU(),
            nn.AdaptiveAvgPool1d(1),
            nn.Flatten(),
            nn.Linear(16, 2)  # 注意输出通道对应修改
        )
...
class MEGPPIS(nn.Module):
    def __init__(self, nlayers, nfeat, nclass, dropout):
