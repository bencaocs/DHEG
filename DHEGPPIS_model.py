...
from graph_unet import Subgraphnet,Initializer
...
class ProDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if ADD_NODEFEATS == 'all' or ADD_NODEFEATS == 'psepose_embedding' or ADD_NODEFEATS == 'seqstr' or ADD_NODEFEATS =='comattention':
            dist_abs = torch.sqrt(torch.sum(pos * pos, dim=1)).unsqueeze(-1) / self.dist  
            dist_rel = torch.sqrt(torch.sum(pos1 * pos1, dim=1)).unsqueeze(-1) / self.dist  
